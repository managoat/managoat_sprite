...
| Hosted macOS CI and Intel build | Both jobs in run 34228092515 passed integration, native fleet, extraction and archive checks; both downloaded checksums verified and hosted ARM app launched locally | Native packaging verified on Apple Silicon and Intel; PR checks track the latest revision |
| Clean second Mac | Hosted Apple Silicon runner launches extracted native WebKit and exercises UI/state/cleanup with system-only PATH; its downloaded app also passes locally | Separate-Mac runner and cross-machine archive launch verified; Gatekeeper distribution remains separate |
...
the existing real ACP tests exercise ask/allow/deny and narrowed policies. It subsequently shipped in service `0.1.1`, as qualified below.
...
## Service 0.1.1 and hosted Mac qualification

The approval fix shipped in the [service 0.1.1 preview](https://github.com/managoat/manasprites/releases/tag/v0.1.1).
Both Linux architecture jobs in [release run 34228314561](https://github.com/managoat/manasprites/actions/runs/34228314561)
passed 31 service tests, all 48 installer/CLI tests, archive checksums and embedded
runtime launch. The exact AMD64 draft archive then installed on a fresh disposable
Sprite, preserved a seeded workspace file, refused unauthenticated API access,
and passed authenticated readiness. Paid Codex work emitted an actual approval
request using the built-in human-review default. The desktop's allow-once answer
was acknowledged, the tool wrote its file afterward, and its attention marker
cleared. No reviewer environment override was present.

The same archive upgraded an existing disposable service from 0.1.0. Its service
key, workspace bytes and conversation IDs survived. A paid follow-up in an
existing conversation recalled the earlier task without reading files or calling
tools. These results preceded publication. Desktop provisioning now pins 0.1.1;
the rebuilt app passes 24 desktop tests, the native fleet walkthrough and the
full extracted ZIP smoke test. That rebuilt package then created another fresh
Sprite through the public 0.1.1 installer, with no local archive override, and
verified authenticated readiness. A real reject-once approval answer completed
through the desktop, left the requested file absent, and cleared its marker.

The Apple Silicon job in [desktop run 34228092515](https://github.com/managoat/manasprites/actions/runs/34228092515)
passed on source `47f1301`: 24 desktop tests, actual native fleet controls, the
extracted-archive checks and artifact upload. Its downloaded archive matched its
SHA-256 report and passed native WebKit, SQLite persistence, exclusive ownership,
normal exit and SIGKILL cleanup checks on the development Mac as well. This
establishes a separate Mac runner and cross-machine artifact launch. That hosted
source predates the service-pin update; the latest pin and compatibility metadata are verified locally, and PR checks
track their follow-up hosted run. Neither check establishes notarized Gatekeeper
distribution. The Intel job subsequently passed the same integration, native fleet and extracted-archive checks. Its downloaded checksum and all 21 native deployment targets were verified as well.

All four additional qualification Sprites were deleted after checking each
saved Sprite ID, organization and unique operation label; API reads verified
404 afterward. The private temporary credentials, account metadata, SQLite
state, logs and probe scripts were removed. The user-supplied key file was
preserved. No pre-existing Sprite was modified.

A further archive audit found that the shell declared macOS 13 while the
embedded runtime required macOS 15. The app now declares macOS 15, and packaging
checks the deployment target in every native binary for the selected architecture.
The earlier local and hosted bundles were both refused by this new check; all
23 local and 21 hosted native binaries were inspected and their maximum required
version was macOS 15. The corrected local archive now passes this check and the full extracted
native smoke test, and its report records `minimum_macos: 15.0`. The hosted Intel
archive also contains 21 native binaries whose maximum minimum version is 15.

## Evidence still needed

Claude provider work remains unqualified because the supplied Anthropic key
returned a credit-balance error. Codex approval routing is published in service
`0.1.1` and selected by desktop provisioning; its fresh-install verification
passed against real paid inference.

Distribution still needs a configured Developer ID/notarization path and a
published desktop artifact. Hosted Apple Silicon/Intel and cross-machine launch
checks have passed; PR checks track final source validation.
